...
namespace RageEngine.LQDB
{
	...
	public class LocalityQueryProximityDatabase<T> : IProximityDatabase<T> where T : class
	{
		...
		private sealed class TokenType : ITokenForProximityDatabase<T>
		{
            ...
            public void UpdateForNewPosition(Point3D p)
			{
				_lq.UpdateForNewLocation(_proxy, p);
			}

			// find all neighbors within the given sphere (as center and radius)
            public void FindNeighbors(Point3D center, int radius, List<T> results)
			{
			...
			// clientQueryState
            private static void perNeighborCallBackFunction(Object clientObject, int distanceSquared, Object clientQueryState)
			{
	    ...
	    readonly LocalityQueryDatabase _lq;
        public readonly Point3D dimensions;
        public readonly Point3D divisions;

		// constructor
        public LocalityQueryProximityDatabase(Point3D dimensions, Point3D divisions)
		{
            this.dimensions = dimensions;
            this.divisions = divisions;

            _lq = new LocalityQueryDatabase(Point3D.Zero, dimensions, divisions.X, divisions.Y, divisions.Z);
