...
namespace RageEngine.LQDB
{
	...
	class LocalityQueryDatabase
	{
		...
		public delegate void LQCallBackFunction(Object clientObject, int distanceSquared, Object clientQueryState);
		 ...
		 * contents.
		 */
		public LocalityQueryDatabase(Point3D origin, Point3D size, int divx, int divy, int divz)
		{
			_origin = origin;
			_size = size;
			_divX = divx;
		...
		public void UpdateForNewLocation(ClientProxy obj, Point3D position)
		{
	    ...
	    private /*lqClientProxy*/int BinForLocation(Point3D position)
		{
		...
		/* Apply an application-specific function to all objects in a certain
		   ...
		   bins of interest. */
        public void MapOverAllObjectsInLocality(Point3D center, int radius, LQCallBackFunction func, Object clientQueryState)
		{
			int partlyOut = 0;
        ...
        private static void TraverseBinClientObjectList(ClientProxy co, int radiusSquared, LQCallBackFunction func, Object state, Point3D position)
		{
			while (co != null)
			{
				// compute distance (squared) from this client obj to given
				// locality sphere's centerpoint
				Point3D d = position - co.Position;
				int distanceSquared = d.LengthSquared();
		...
		public void RemoveAllObjects()
		{
			for (int i = 0; i < _bins.Length; i++)
			{
				RemoveAllObjectsInBin(_bins[i]);
			}
		}

		/* public helper function */
		void RemoveAllObjectsInBin(ClientProxy bin)
		{
			while (bin != null)
			{
				RemoveFromBin(bin);
		...
		struct FindNearestState
		{
            ...
            public int minDistanceSquared;

		}

        static void FindNearestHelper(Object clientObject, int distanceSquared, Object clientQueryState)
		{
        ...
        public Object FindNearestNeighborWithinRadius(Point3D center, int radius, Object ignoreObject)
		{
			/* initialize search state */
			FindNearestState lqFNS;
			lqFNS.nearestObject = null;
			lqFNS.ignoreObject = ignoreObject;
            lqFNS.minDistanceSquared = int.MaxValue;
